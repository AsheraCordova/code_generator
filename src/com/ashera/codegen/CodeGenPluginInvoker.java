...
public class CodeGenPluginInvoker extends CodeGenBase {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < 50; i++) {
			String copyFileFromUrl = properties.getProperty("copyfromhttp.files." + i);
			if (copyFileFromUrl != null) {
				String httpUrl =  copyFileFromUrl.split("##")[0];
				String fileToCopy = copyFileFromUrl.split("##")[1];
				File file = readHttpUrlAsString(httpUrl, fileToCopy.substring(fileToCopy.lastIndexOf("/") + 1), "cache/", false);
				gen.copyFileContent(file, new File(fileToCopy), false, properties);
			} else {
				break;
			}
		}
